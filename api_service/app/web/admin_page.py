...
<body class="min-h-screen bg-slate-950 text-slate-100 antialiased">
  <div class="mx-auto flex min-h-screen max-w-[1600px]">
    <aside class="hidden w-64 shrink-0 border-r border-slate-800 bg-slate-950 p-5 lg:block">
      ...
      <div class="mt-10 rounded-xl border border-slate-800 bg-slate-900/50 p-3 text-xs leading-5 text-slate-400">所有管理请求均受 HTTP Basic Auth 保护。API Key 的明文仅在创建后显示一次。</div>
    </aside>
    <main class="min-w-0 flex-1 p-5 sm:p-8">
      ...
      <section data-panel="overview" class="panel space-y-6">
        <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <article class="rounded-2xl border border-slate-800 bg-slate-900/60 p-5"><p class="text-sm text-slate-400">可用 MCP 工具</p><p class="mt-3 text-3xl font-bold" id="stat-tools">—</p><p class="mt-2 text-xs text-slate-500">当前激活的工具目录</p></article>
      ...
      <section data-panel="roles" class="panel hidden space-y-6">
        <div class="grid gap-6 xl:grid-cols-[380px_1fr]"><form id="role-form" class="rounded-2xl border border-slate-800 bg-slate-900/60 p-5"><h2 class="text-lg font-semibold">创建角色</h2><p class="mt-1 text-sm text-slate-400">例如 finance_reader、hr_reader。</p><label class="mt-5 block text-sm text-slate-300">角色名<input required name="name" pattern="[A-Za-z0-9_-]+" class="field mt-2" placeholder="finance_reader"></label><label class="mt-4 block text-sm text-slate-300">说明<textarea name="description" rows="3" class="field mt-2" placeholder="可查询财务知识库"></textarea></label><button class="primary-btn mt-5 w-full">创建角色</button></form>
          <div class="rounded-2xl border border-slate-800 bg-slate-900/60 p-5"><h2 class="text-lg font-semibold">给角色分配工具</h2><p class="mt-1 text-sm text-slate-400">保存后，绑定该角色的 API Key 会获得这些工具。</p><select id="role-select" class="field mt-5"></select><div id="role-tools" class="mt-4 grid gap-3 sm:grid-cols-2"></div><button id="save-role-tools" class="primary-btn mt-5">保存工具授权</button></div></div>
  ...
  <script>
    ...
    function serverName(id){return state.servers.find(s=>s.id===id)?.name||'未知服务'}
    function render(){
      ...
      $('#tool-count').textContent=`${state.tools.length} 个工具`;$('#tool-list').innerHTML=state.tools.length?state.tools.map(t=>`<article class="rounded-xl border border-slate-800 bg-slate-950 p-4"><p class="text-xs text-emerald-400">${esc(serverName(t.server_id))}</p><h3 class="mt-1 font-mono text-sm font-semibold">${esc(t.name)}</h3><p class="mt-2 text-sm text-slate-400">${esc(t.description||'暂无描述')}</p></article>`).join(''):'<p class="col-span-full rounded-xl border border-dashed border-slate-700 p-6 text-center text-sm text-slate-500">登记并同步 MCP 服务后，工具会出现在这里。</p>';
    ...
    async function syncServer(id,button){try{button.disabled=true;button.textContent='同步中…';const tools=await api(`/mcp-servers/${id}/sync-tools`,{method:'POST'});toast(`已同步 ${tools.length} 个工具`);await loadAll()}catch(e){toast(e.message,'error')}finally{button.disabled=false;button.textContent='同步工具'}}
    ...
    $('#server-form').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target);try{await api('/mcp-servers',{method:'POST',body:JSON.stringify(Object.fromEntries(f))});e.target.reset();toast('MCP 服务已登记');await loadAll()}catch(err){toast(err.message,'error')}};
    $('#role-form').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target);try{await api('/roles',{method:'POST',body:JSON.stringify(Object.fromEntries(f))});e.target.reset();toast('角色已创建');await loadAll()}catch(err){toast(err.message,'error')}};
...
(() => {
  ...
  document.querySelector('#save-feishu-user').onclick = async () => {
    ...
    try { await api(`/feishu-users/${editingUser.id}`, {method:'PUT', body:JSON.stringify({role_ids:checked('#feishu-user-roles'), extra_tool_ids:checked('#feishu-user-tools'), is_active:document.querySelector('#feishu-user-active').checked})}); document.querySelector('#feishu-user-dialog').close(); await loadFeishuUsers(); toast('飞书用户权限已保存'); } catch (error) { toast(error.message, 'error'); }
  };
